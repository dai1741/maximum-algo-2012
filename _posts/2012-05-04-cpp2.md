...
vectorのイテレータは配列のポインタとほぼ同等に扱うことができる。
...
- `lower_bound(begin, end, value)` `upper_bound(begin, end, value)` イテレータが示す範囲で2分探索
  - 二分探索はソート済みの配列から効率よく値を検索するアルゴリズムのこと（2年生はもう講義でやったのかな）
  - 戻り値はイテレータとなる
...
- [complex](http://www.cplusplus.com/reference/std/complex/)：複素数
...
- [functional](http://www.cplusplus.com/reference/std/functional/)：関数型言語ライクな関数
...
C言語では「ローカル変数宣言は関数の先頭にまとめておかないといけない」と習ったかもしれませんが、
