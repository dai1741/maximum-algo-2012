...
<!--
まず再帰関数の引数を考える必要があります。再帰ですべての品物を走査する必要があるので、現在の品物のインデックス`i`を保持しておき必要がありそうです。

また、ある品物を入れるか入れないか・入れられるかを判別するには、現時点でのナップサックの残り容量`j`も保持しておく必要があります。

その時点での価値の総和も引数に入れたほうがいいようにも感じますが、これは再帰関数の戻り値で表現できるので入れる必要はありません。動的計画法では再帰関数の引数を少なく保って状態数を抑えることが重要なので、引数は`i`と`j`の2つだけにしましょう。

これらのことを考慮すると、次のような再帰プログラムを実装することができます。
-->
...
<!--（Wが極端に大きいときは半分全列挙という方法を使います。詳しくは<abbr title="プログラミングコンテストチャレンジブック">蟻本</abbr>の147ページ参照）-->
