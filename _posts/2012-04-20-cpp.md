...
上の例では入出力が少し変わっただけだが、C++の標準ライブラリ（[STL](http://ja.wikipedia.org/wiki/Standard_Template_Library)とも呼ばれる）を使うとコードが一気に簡略化できることもある。
...
- 文字列を表すのにchar配列ではなくC++のstirng型を使っっているので、`==`による文字列比較ができるようになった。
  - これは[演算子オーバーロード](http://www.geocities.jp/ky_webid/cpp/language/017.html)というC++の機能の効果。
  - 余談だが、等号演算子`=`もオーバーロードされているのでCではいとどおりに動かない`str = str2;`でも文字列の代入ができる。
...
bccとgccは基本的な部分ではほとんど変わりはないが、細かい違いがある。
